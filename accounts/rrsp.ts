...
export class RRSP extends Account{
    ...
    increaseContributionRoom(grossIncome: number){
        this.contributionRoom += Math.min(grossIncome * contributionRoomMultiplier, maxContributionRoomPerYear)
    }

    @NonNegativeFirstArg    
    addMoney(amount: number): TransactionReturn{
        if (amount < this.contributionRoom) throw Error("Cannot exceed contribution room in RRSP")
        this.value += amount
    ...
    withdrawal(amount: number): TransactionReturn{
        if (amount < this.value) throw Error("Cannot withdrawal more than exists in RRSP")
        this.value -= amount;
        return {moneyOut: amount, taxableIncome: amount, realizedCapitalGains: 0}
    }
}