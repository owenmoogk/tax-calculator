...
function applyTax(income: number, capitalGains: number, capitalGainsInclusionRate: number, taxBrackets: TaxBracket[]){
	var totalTax = 0
...
export function calculateTaxOwed(income: number, capitalGains: number): TransactionReturn {
	if (capitalGains < 0){
		throw new Error("Can't claim a negative income.");
	}
	var totalTax = 0
	totalTax += applyTax(income, capitalGains, capitalGainsInclusionRateCanada, canadaTaxBrackets)
	totalTax += applyTax(income, capitalGains, capitalGainsInclusionRateOntario, ontarioTaxBrackets)

    return {moneyOut: -totalTax, taxableIncome: -income, realizedCapitalGains: -capitalGains};
}